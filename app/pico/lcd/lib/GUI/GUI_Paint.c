...
void Paint_DrawString_EN(UWORD Xstart, UWORD Ystart, const char* pString, sFONT* Font, UWORD Color_Foreground, UWORD Color_Background) {
	if (Font == &Font24) {
		Paint_DrawString_EN_Scaled(Xstart, Ystart, pString, Font, Color_Foreground, Color_Background, 3);
		return;
	}
...
void Paint_DrawChar(UWORD Xpoint, UWORD Ypoint, const char Acsii_Char, sFONT* Font, UWORD Color_Foreground, UWORD Color_Background) {
	...
	const bool invalid_start_position = Xpoint > Paint.Width || Ypoint > Paint.Height;
	if (invalid_start_position) {
		Debug("Paint_DrawChar Input exceeds the normal display range\r\n");
		return;
	}

	// calculate offset into the font lookup table.
...
/******************************************************************************
 * @brief custom draw string that will scale the selected font
 * @param Xstart
 * @param Ystart
 * @param pString
 * @param Font
 ...
 * @param scale
 *****************************************************************************/
void Paint_DrawString_EN_Scaled(UWORD Xstart, UWORD Ystart, const char* pString, sFONT* Font, UWORD Color_Foreground, UWORD Color_Background, int scale) {
	UWORD Xpoint = Xstart;
	UWORD Ypoint = Ystart;
...
//	printf("Paint_DrawString_EN_Scaled: %d %d %s %d %d %d\r\n", Xstart, Ystart, pString, Font->Width, Font->Height, scale);
	...
	const bool invalid_start_position = Xstart > Paint.Width || Ystart > Paint.Height;
	if (invalid_start_position) {
		Debug("Paint_DrawString_EN_Scaled Input exceeds the normal display range\r\n");
		return;
	}
	...
	while (*pString != '\0') {
		...
		if (*pString != ' ') {
			Paint_DrawChar_Scaled(Xpoint, Ypoint, *pString, Font, Color_Foreground, Color_Background, scale);
		}
...
void Paint_DrawChar_Scaled(UWORD Xpoint, UWORD Ypoint, const char Acsii_Char, sFONT* Font, UWORD Color_Foreground, UWORD Color_Background, int scale) {
	UWORD Page, Column;
		...
		for (Column = 0; Column < Font->Width - 0; Column++) {
			...
			for (int x = 0; x < scale; ++x) {
				for (int y = 0; y < scale; ++y) {
					...
					if (FONT_BACKGROUND == Color_Background) {  //this process is to speed up the scan
						if (*ptr & (0x80 >> (Column % 8))) {
							Paint_SetPixel(Xpoint + (Column * scale) + x, Ypoint + (Page * scale) + y, Color_Foreground);
						}
					} else {
						if (*ptr & (0x80 >> (Column % 8))) {
							Paint_SetPixel(Xpoint + (Column * scale) + x, Ypoint + (Page * scale) + y, Color_Foreground);
						} else {
							Paint_SetPixel(Xpoint + (Column * scale) + x, Ypoint + (Page * scale) + y, Color_Background);
						}
					}
				}
			}
